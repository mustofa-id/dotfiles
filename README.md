...
Dry-run by default; `--apply` flag to install. To exclude files from installation, list them in the `.exclude` file.

## Packages

`niri` `waybar` `fuzzel` `swaybg` `swayosd` `swaylock` `swayidle` `mako` `xdg-desktop-portal` `xdg-desktop-portal-gnome` `xdg-desktop-portal-gtk` `alacritty` `foot` `cliphist` `rofimoji` `wl-clipboard networkmanager bluez-utils bluez wf-recorder` `otf-cascadia-code` `ttf-lato`
